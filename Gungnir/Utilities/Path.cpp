...
{
	return ExistFile(ToWString(path));
}
...
{
	return ExistDirectory(ToWString(path));
}
...
string Path::ToString(wstring w_str)
{
	string temp;
	temp.assign(w_str.begin(), w_str.end());
	return temp;
}

wstring Path::ToWString(string str)
{
	wstring temp;
...
void Path::Replace(wstring * str, wstring comp, wstring rep)
{
	wstring temp = *str;
...
const WCHAR* Path::ShaderFilter = L"HLSL file\0*.fx";
...
{
	return IsRelativePath(ToWString(path));
}
