...
{
	return fabsf(value) <= FLT_EPSILON ? true : false;
}

bool Math::IsNearlySame(float v1, float v2)
{
	return IsNearlyZero(v1 - v2);
}

bool Math::IsNearlyZero(double value)
{
	return abs(value) <= DBL_EPSILON ? true : false;
}

bool Math::IsNearlySame(double v1, double v2)
{
	return IsNearlyZero(v1 - v2);
}
...
void Math::MatrixDecompose(const D3DXMATRIX & m, OUT D3DXVECTOR3 & S, OUT D3DXVECTOR3 & R, OUT D3DXVECTOR3 & T)
{
